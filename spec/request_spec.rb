...
describe 'Request handling' do
  ...
  it 'should correctly identify valid AWS requests' do
    ...
    expect(AlexaRuby::valid_alexa?(sample_bad_request)).to be false

    sample_good_request = JSON.parse(File.read('fixtures/sample-IntentRequest.json'))
    expect(AlexaRuby::valid_alexa?(sample_good_request)).to be true
  end
  ...
  it 'transform_keys_to_symbols' do
    ...
    expect(AlexaRuby.transform_keys_to_symbols(string_keys_hash)).to eq(symbol_keys_hash)
  end
end
